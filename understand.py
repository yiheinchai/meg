    ...
    def __init__(
        ...
        def cached_load(subj_id):
            if self.use_npz_cache:
                return fast_load_meg_data(
                    subj_id, cache_dir=npz_cache_dir, base_path=base_path
                )
            else:
        ...
        if use_npz_cache:
            first_data = fast_load_meg_data(
                subject_ids[0], cache_dir=npz_cache_dir, base_path=base_path
            )
        else:
...
if __name__ == "__main__":
    ...
    if USE_WINDOW_CACHE:
        ...
        train_dataset = WindowMEG_Dataset(
            window_cache_dir="meg_windows_cache", transforms=aug_pipeline
        )
    else:
        print("Using LazyMEG_Dataset with window_size=2000, stride=500...")
        train_dataset = LazyMEG_Dataset(
            participants_ids, window_size=2000, transforms=aug_pipeline, stride=500
        )
